...
class SnowflakeId {
  // 静态常量
  private static readonly MAX_DATACENTER = 0x1f // 5位最大值 (31)
  private static readonly MAX_WORKER = 0x1f // 5位最大值 (31)
  private static readonly MAX_SEQUENCE = 0xfff // 12位序列号最大值 (4095)
  ...
  next(cb?: (err: Error | null, id?: Buffer) => void): Buffer | undefined {
    ...
    if (time === this.lastTime) {
      ...
      if (this.seq === 0) this.overflow = true
    } else {
      this.overflow = false
      this.seq = 0
    }
  ...
  private handleClockBackwards(time: number, cb?: (err: Error | null, id?: Buffer) => void): undefined {
  ...
  private handleOverflow(cb?: (err: Error | null, id?: Buffer) => void): undefined {
    if (cb) {
